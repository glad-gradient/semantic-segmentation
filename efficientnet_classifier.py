...
if __name__ == '__main__':
    ...
    train_generator = create_aug_generator(make_image_generator(train_df), data_generator)
    valid_generator = create_aug_generator(make_image_generator(valid_df), data_generator)

    K.set_image_data_format('channels_last')

    if LOAD_MODEL:
        model = load_model(f'{PATH_CHECKPOINT}/{LOAD_MODEL}.hdf5')
